...
urllist.urlrefactor = function (codes, js, callback) {
  ...
  var urlpattern = urlpattern || create_url_pattern(js);
...
var create_url_pattern = function (js) {
  ...
  return js ? new RegExp('(?:src|href)\\s*:\\s*[\'"]((' + dir_rel_paths.join('|') + ')[^?\'"]+)', 'gim') : new RegExp('(?:url\\s*\\(\\s*|src\\s*=\\s*|href\\s*=\\s*|:\\s*)[\'"]?((' + dir_rel_paths.join('|') + ')[^\\?\'"#>)]+)', 'img');
};