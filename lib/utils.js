...
exports.css = function(files, callback){
  async.mapSeries(files, function(file, done){
    ...
    fs.readFile(file, 'utf8', function(err, content){
      ...
      switch (extname){
        case '.less':
          return less.render(content, {paths: paths, filename: filename}, done);
        case '.scss':
    ...
    if (profile.autoprefixer){
      codes = autoprefixer({browsers: profile.autoprefixer, cascade: profile.beautify}).process(codes).css;
    }
