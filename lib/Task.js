...
var filesUtils = require('transforms');
...
function Task(stack){
  ...
  if (config.filesWatch) {
    ...
    // prepare the file-mapping properties for filecopy later
    //this._patterns = this._patterns.concat(filesUtils.getMappedPatterns(this._mappings));
...
var files_copy = function(prev, callback) {
  async.each(this._mappings, function(mapping, done) {
    ...
    filesUtils.moveFiles(mapping.list, option, function(err) {

      if (err) {
        utils.warn(err);
        utils.warn('failed to copy %s', mapping.from);
      }
      else {
        utils.message('copy files successful. %s', mapping.from);
      }

      done();
    });
  }, callback.bind(null, null, prev));
};
