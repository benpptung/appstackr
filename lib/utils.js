...
exports.concat = function (files, callback) {
  ...
  async.mapSeries(files,
    function (file, done) {
      ...
      config.jsEXNames.indexOf(ext) >= 0 ?
        concat_hook_jsEX(file, done) :
...
exports.uglify = function (codes, callback) {
  ...
  // remove this someday when uglify-js upgraded for new browsers
  exports.jsEX(codes, function(err, _codes) {
    if (err) return callback(err);
    exports.useChilds('uglifyjs.js', _codes, callback);
  });
